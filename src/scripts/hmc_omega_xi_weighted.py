...
With weight thresholding to reduce memory usage.
"""
...
# CRITICAL: Weight threshold to reduce memory usage
# Sources with occupation probability below this are excluded
WEIGHT_THRESHOLD = 0.01  # 1% threshold - adjust if needed
...
# ============================================================
# Extract halo catalog arrays
# ============================================================
print("\nExtracting halo catalog arrays...")
...
# All host data (before filtering)
host_pos_all = np.stack([halo_x[host_mask], halo_y[host_mask], halo_z[host_mask]], axis=1)
host_mvir_all = halo_mvir[host_mask]
host_rvir_all = halo_rvir[host_mask]
host_axis_all = np.stack([ax_x[host_mask], ax_y[host_mask], ax_z[host_mask]], axis=1)
host_axis_all = host_axis_all / (np.linalg.norm(host_axis_all, axis=-1, keepdims=True) + 1e-12)
host_hostid_all = halo_hostid[host_mask]

n_hosts_all = len(host_mvir_all)
print(f"  Total hosts (before filtering): {n_hosts_all}")

# Build host index mapping
host_index_of = {int(k): i for i, k in enumerate(host_hostid_all.tolist())}

# Subhalo data (before filtering)
sub_pos_all = np.stack([halo_x[sub_mask], halo_y[sub_mask], halo_z[sub_mask]], axis=1)
sub_mvir_all = halo_mvir[sub_mask]
sub_hostid_all = halo_hostid[sub_mask]

# Map subhalos to host indices
sub_host_idx_list = []
sub_keep_mask_init = []
for i, k in enumerate(sub_hostid_all):
    j = host_index_of.get(int(k), None)
    if j is not None:
        sub_host_idx_list.append(j)
        sub_keep_mask_init.append(True)
    else:
        sub_keep_mask_init.append(False)

sub_keep_mask_init = np.array(sub_keep_mask_init)
sub_host_idx_all = np.array(sub_host_idx_list, dtype=np.int64)
sub_pos_all = sub_pos_all[sub_keep_mask_init]
sub_mvir_all = sub_mvir_all[sub_keep_mask_init]

n_subs_all = len(sub_mvir_all)
print(f"  Total subhalos (with valid hosts, before filtering): {n_subs_all}")

# ============================================================
# Filter sources by occupation probability at fiducial HOD
# ============================================================
print(f"\nFiltering sources by occupation probability (threshold={WEIGHT_THRESHOLD})...")

# Compute occupation probabilities at fiducial HOD
mean_N_cen_fid = np.array(Ncen(jnp.array(host_mvir_all), 
                                TRUE_PARAMS['logMmin'], 
                                TRUE_PARAMS['sigma_logM']))
mean_N_sat_fid = np.array(Nsat(jnp.array(host_mvir_all), 
                                TRUE_PARAMS['logMmin'], TRUE_PARAMS['sigma_logM'],
                                TRUE_PARAMS['logM0'], TRUE_PARAMS['logM1'], 
                                TRUE_PARAMS['alpha']))

# Filter hosts: keep those with significant central occupation
host_keep = mean_N_cen_fid > WEIGHT_THRESHOLD
n_hosts_kept = np.sum(host_keep)

# Create mapping from old to new host indices
old_to_new_host = np.full(n_hosts_all, -1, dtype=np.int64)
old_to_new_host[host_keep] = np.arange(n_hosts_kept)

# Filter subhalos: keep those whose host is kept AND has significant satellite occupation
sub_host_nsat = mean_N_sat_fid[sub_host_idx_all]
sub_host_kept = host_keep[sub_host_idx_all]
sub_keep = sub_host_kept & (sub_host_nsat > WEIGHT_THRESHOLD)
n_subs_kept = np.sum(sub_keep)

print(f"  Hosts: {n_hosts_all} -> {n_hosts_kept} ({100*n_hosts_kept/n_hosts_all:.1f}%)")
print(f"  Subhalos: {n_subs_all} -> {n_subs_kept} ({100*n_subs_kept/n_subs_all:.1f}%)")
print(f"  Total sources after filtering: {n_hosts_kept + n_subs_kept}")

# Apply filters to create final arrays
HOST_POS = jnp.array(host_pos_all[host_keep])
HOST_MVIR = jnp.array(host_mvir_all[host_keep])
HOST_RVIR = jnp.array(host_rvir_all[host_keep])
HOST_AXIS = jnp.array(host_axis_all[host_keep])

SUB_POS = jnp.array(sub_pos_all[sub_keep])
SUB_MVIR = jnp.array(sub_mvir_all[sub_keep])
SUB_HOST_IDS = jnp.array(old_to_new_host[sub_host_idx_all[sub_keep]], dtype=jnp.int32)

N_HOSTS = n_hosts_kept
N_SUBS = n_subs_kept

print(f"  Final N_hosts = {N_HOSTS}")
print(f"  Final N_subhalos = {N_SUBS}")
...
# ============================================================
# Pre-compute neighbor pairs for filtered galaxy positions
# ============================================================
print("\nPre-computing neighbor pairs for filtered sources...")
...
print(f"  Total sources = {len(ALL_POS)}")
print(f"  N_pairs = {len(i_idx_all)}")

# Estimate memory usage
mem_pairs_gb = len(i_idx_all) * 4 * 2 / 1e9
mem_diff_gb = len(i_idx_all) * 3 * 4 / 1e9
print(f"  Estimated pair index memory: {mem_pairs_gb:.2f} GB")
print(f"  Estimated diff array memory: {mem_diff_gb:.2f} GB")

if mem_diff_gb > 30:
    print(f"\n  WARNING: Memory usage may exceed GPU capacity!")
    print(f"  Consider increasing WEIGHT_THRESHOLD or reducing r_max")
...
def main():
    ...
    print(f"N_ORIENTATION_SAMPLES = {N_ORIENTATION_SAMPLES}")
