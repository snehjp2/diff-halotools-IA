...
class DiffHalotoolsIA:
    ...
    def plot_diagnostic(
        ...
        *,
        ...
        """
        Visualize galaxy field and IA statistics using halotools.{tpcf, ed_3d, ee_3d}.

        Panels:
        - If `orig_catalog is None` (default): 4 panels
            [0] Generated density (x,y)
            [1] xi(r) 2PCF (generated)
            [2] omega(r) ED-3D (generated)
            [3] eta(r)   EE-3D (generated)

        - If `orig_catalog` provided: 5 panels
            [0] Original density (x,y)
            [1] Generated density (x,y)
            [2] xi(r): Original vs Generated
            [3] omega(r): Original vs Generated
            [4] eta(r):   Original vs Generated

        Args:
            - orig_catalog : (N, >=6) array [x,y,z,nx,ny,nz] for the reference catalog, optional.
            - L            : Periodic box size (Mpc/h). If None, no periodic wrapping.
            - nbins        : 2D density map resolution (per axis).
            - rbins_2pcf   : 1D array of r-bin edges for 2PCF. Defaults to logspace 0.1..16 with 20 bins.
            - rbins_ia     : 1D array of r-bin edges for ED/EE. Defaults to logspace 0.1..16 with 20 bins.
            - num_threads  : Halotools thread count.
        figsize      : Matplotlib (width, height). Defaults to (18, 4) for 5 panels; (16, 4) for 4 panels.
        ...
        try:
            ...
            raise RuntimeError(
                "plot_diagnostic requires halotools. Install halotools or skip plotting."
            ) from e

        import matplotlib.pyplot as plt

        # Ensure generated catalog exists
        gal_cat = self.return_catalog()
        if gal_cat is None or gal_cat.numel() == 0:
            raise RuntimeError("Generated catalog is empty.")

        # --- pull arrays (generated) ---
        gen = gal_cat.detach().cpu().numpy()  # (Ng, 6)
        gen_pos = gen[:, :3]
        gen_ori = gen[:, 3:6]
        ...
        if has_orig:
            ...
            if orig.ndim != 2 or orig.shape[1] < 6:
                raise ValueError(
                    "orig_catalog must have shape (N, >=6): positions (0:3) and orientations (3:6)."
                )
            orig_pos = orig[:, :3]
            orig_ori = orig[:, 3:6]

        # --- periodic wrap (for density maps & pair stats) ---
        if L is not None:
        ...
        # --- density maps (shared bin edges if comparing) ---
        if has_orig:
        ...
        if rbins_2pcf is None:
            rbins_2pcf = np.logspace(np.log10(0.1), np.log10(16.0), 20)
        if rbins_ia is None:
            rbins_ia = rbins_2pcf  # reuse by default
        ...
        if has_orig:
            ...
            axs[col].set_xlabel(r"x $[\,\mathrm{Mpc}/h\,]$")
            axs[col].set_ylabel(r"y $[\,\mathrm{Mpc}/h\,]$")
            col += 1
...
if __name__ == "__main__":
    ...
    fig, axs = builder.plot_diagnostic(orig_catalog=original_catalog)
    plt.show()
